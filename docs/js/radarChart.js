function RadarChart() {
   ...
   var events = {
      ...
      'axisLegend': { 'mouseover': axisLegendOver, 'mouseout': axisLegendOut, 'mouseclick': axisLegendClick },
   ...
   var legend_toggles = [];
   var radial_calcs = {};
   var Format = d3.format('g'); // Percentage formatting
            ...
            update = function() {
                ...
                update_radarCircle
                   ...
                   .style("fill-opacity", function(d, i, j) { 
                      ...
                      return options.areas.filter.indexOf(key) >= 0 ? 0 : 1; // <-- radar dots changed from 0.8 to 1
                   })
                   .attr("r", options.areas.dotRadius)
   ...
   function areaMouseover(d, i, self) {
      ...
      chart_node.selectAll("." + options.class + "RadarArea")
         .transition().duration(200)
			.style("fill-opacity", function(d, i, j) {
            //return options.areas.filter.indexOf(d.key) >= 0 ? 0 : 0.1; <-- Changed from 0.1 to 0.0
            return options.areas.filter.indexOf(d.key) >= 0 ? 0 : 0.0;
          }) 
      ...
      //Bold legend title on mouseover
      d3.select(d.key)
   ...
   function areaMouseout(d, i, self) {
      ...
      //Remove bold for legend title on mouseout
      d3.select(self)
      .transition().duration(0)
   ...
   // on mouseover for axisLegendOver 
   function axisLegendOver(d, i, self) {
     chart_node.select('[key="'+d.axis+'"]').select('text')
         .style('font-weight',700)
         .style('cursor', 'pointer');
   }

   // on mouseover for axisLegendOut 
   function axisLegendOut(d, i, self) {
      chart_node.select('[key="'+d.axis+'"]').select('text')
            .style('font-weight',300);
   }

   // on click for axisLegendClick 
   function axisLegendClick(d, i, self) {
      ...
      //var modal_heading = 
      document.getElementById("modal_heading")
         .textContent=d.axis;
      //var modal_sub = 
